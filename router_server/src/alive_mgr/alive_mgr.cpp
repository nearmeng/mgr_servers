...
BOOL CAliveMgr::init(BOOL bResume)
{
	...
	nRetCode = m_ShmAliveData.init(sstdRouterAliveData, bResume);
...
void CAliveMgr::mainloop()
{
	...
	for (int32_t i = 0; i < m_pAliveData->nServerCount; i++)
	{
		...
		if (rServerInfo.bAlive && nCurrTime > rServerInfo.dwLastHeartBeatTime + g_ServerConfig.nHeartBeatTimeOut / 1000)
		{
	...
	if (nCurrTime > m_pAliveData->dwLastReportTime + g_ServerConfig.nRouterReportInterval / 1000 &&
