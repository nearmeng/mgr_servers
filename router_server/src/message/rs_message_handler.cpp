...
BOOL CRSMessageHandler::init(int32_t nTbusHandle, int32_t nTbusAddr, BOOL bResume)
{
	...
	nRetCode = m_ShmMsgQueue.init(sstdRouterMsgQueue, bResume);
	LOG_PROCESS_ERROR(nRetCode);
	
	nRetCode = m_ShmServiceMgrData.init(sstdRouterServiceMgr, bResume);
	LOG_PROCESS_ERROR(nRetCode);
	...
	if (!bResume)
	{
		...
		for (int32_t nIndex = 0; nIndex < nPeerCount; nIndex++)
		{
			...
			else if (nServerType == tbus_get_type(CMGApp::instance().get_tbus_addr()))
			{
			}
			else
			{
...
BOOL CRSMessageHandler::mainloop(void)
{
	...
	if (nCurTime > m_pShmServiceMgrData->dwLastHeartbeatTime + g_ServerConfig.nRouterHeartBeatInterval / 1000)
	{
		do_heartbeat();
...
void CRSMessageHandler::on_router_obj_report(const char* pBuffer, size_t dwSize, int32_t nSrcAddr)
{
	...
	if(g_ServerConfig.bRouterLogEnable)
...
void CRSMessageHandler::on_router_send_by_service_type(const char* pBuffer, size_t dwSize, int32_t nSrcAddr)
{
	...
	if (tbus_get_type(nSrcAddr) != tbus_get_type(CMGApp::instance().get_tbus_addr()))
	{
...
SERVER_INFO CRSMessageHandler::_server_data_to_server_info(const SERVER_DATA& data)
{
	SERVER_INFO info;

	info.nLoadValue = data.nLoadValue;
	info.nRouterAddr = data.nRouterAddr;
	info.nState = data.nState;
	info.nTbusAddr = data.nTbusAddr;
...
void CRSMessageHandler::on_service_mgr_sync_server_info(const char* pBuffer, size_t dwSize, int32_t nSrcAddr)
{
	...
	{
		SERVER_INFO info = _server_data_to_server_info(msg->ServerInfo[i]);

		nRetCode = CRouterMgr::get_instance().add_server_info(info);
		LOG_PROCESS_ERROR(nRetCode);
...
void CRSMessageHandler::on_service_mgr_server_info_ntf(const char* pBuffer, size_t dwSize, int32_t nSrcAddr)
{
	...
	for (int32_t i = 0; i < msg->nServerInfoCount; i++)
	{
		...
		{
			*pServerInfo = _server_data_to_server_info(msg->ServerInfo[i]);
		}
