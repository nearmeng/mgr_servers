...
void CRedisCli::mainloop(void)
{
    ...
    case rcsAuthed:
        ...
        // ping
        if (nCurrTime > m_nPingSec + g_ServerConfig.nDBPingInterval / 1000)
        {
            nRetCode = _ping();
            LOG_CHECK_ERROR(nRetCode);
        }

        if (nCurrTime > m_nPongSec + g_ServerConfig.nDBPingTimeout / 1000)
        {
    ...
    case rcsDisConnected:
        ...
        //reconnect
        if (nCurrTime > m_nDisConnectSec + g_ServerConfig.nDBReConnectInterval / 1000)
        {
...
BOOL CRedisCli::sync_connect(void)
{
    ...
    while (true)
    {
        ...
        {
            if (qwCurrTime - qwStartTime >= g_ServerConfig.nSyncConnectTimeout)
            {
...
BOOL CRedisCli::sync_disconnect(void)
{
    ...
    while (true)
    {
        ...
        {
            if (qwCurrTime - qwStartTime >= g_ServerConfig.nSyncDisConnectTimeout)
            {
...
CRedisCli::CALLBACK_DATA* CRedisCli::_prepare_callback_data(int32_t nExecuteAddr, int32_t nCmdID, uint64_t qwCoroID, const char* pUserData, size_t dwDataLen)
{
    ...
    pCallBackData->dwExpireTime = CTimeMgr::instance().get_time_sec() + g_ServerConfig.nDBReqExpireTime / 1000;
