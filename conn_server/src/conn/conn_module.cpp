...
#include "proto/common_proto.pb.h"
...
BOOL CConnModule::init(BOOL bResume)
{
    ...
    nRetCode = m_SessionMgr.init(sstdSession, g_ServerConfig.nInitSessionMgrCount, bResume);
...
BOOL CConnModule::_init_tconnd_addr(void)
{
	...
	for (int32_t nIndex = 0; nIndex < nPeerCount; nIndex++)
	{
		...
		if (nServerType == g_ServerConfig.nTconndServerType)
		{
