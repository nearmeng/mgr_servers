...
#include "router_def.h"
#include "define/server_def.h"
#include "config/global_config.h"
#include "tbus/tbus_wrapper.h"

#include "common_message.h"
#include "service_manage_message.h"
...
BOOL CSMSMessageHandler::init(int32_t nTbusHandle, int32_t nTbusAddr, const char* pcszTag, BOOL bResume)
{
	...
	nRetCode = m_ShmMsgQueue.init(sstdSMMsgQueue, bResume);
	LOG_PROCESS_ERROR(nRetCode);
		
	nRetCode = m_ShmMessageData.init(sstdSMMsgData, bResume);
	LOG_PROCESS_ERROR(nRetCode);
	...
	if (!bResume)
	{
		...
		for (int32_t nIndex = 0; nIndex < nPeerCount; nIndex++)
		{
			...
			if (nServerType == tbus_get_type(CMGApp::instance().get_tbus_addr()))
			{
			...
			else if (nServerType == g_ServerConfig.nRouterServerType)
...
BOOL CSMSMessageHandler::uninit(void)
{
	...
	{
		nRetCode = do_sync_end_service(tbus_get_type(CMGApp::instance().get_tbus_addr()));
...
BOOL CSMSMessageHandler::do_sync_end_service(int32_t nServerType)
{
	...
	LOG_PROCESS_ERROR(nServerType > 0 && nServerType < MAX_SERVICE_COUNT);

	msg.wMsg = service_mgr_end_service;

	if (nServerType == g_ServerConfig.nRouterServerType)
	{
		nRetCode = broadcast_to_router(&msg, sizeof(msg));
		LOG_PROCESS_ERROR(nRetCode);
	}
	else if (nServerType == g_ServerConfig.nServiceMgrServerType)
	{
...
SERVER_DATA CSMSMessageHandler::_server_info_to_server_data(const SERVER_INFO& info)
{
	SERVER_DATA data;

	data.nLoadValue = info.nLoadValue;
	data.nRouterAddr = info.nRouterAddr;
	data.nState = info.nState;
	data.nTbusAddr = info.nTbusAddr;
...
BOOL CSMSMessageHandler::do_sync_server_info(int32_t nDstAddr)
{
	...
	for (int32_t i = 0; i < nServerCount; i++)
	{
		...
		if(pServerInfo)
			pMsg->ServerInfo[i] = _server_info_to_server_data(*pServerInfo);
	}
...
void CSMSMessageHandler::on_router_unregister_report(const char* pBuffer, size_t dwSize, int32_t nSrcAddr)
{
	...
	if (nServerType == g_ServerConfig.nGameServerType)
	{
		...
		pServiceInfo = CServiceMgr::get_instance().get_service_info(nServerType);
		if (pServiceInfo && pServiceInfo->nServerCount == nUnRegisterCount)
		{
			...
			if (raft_service_get_status(m_pRaftMgr) == rsLeader)
			{
				nRetCode = do_sync_end_service(g_ServerConfig.nRouterServerType);
				LOG_CHECK_ERROR(nRetCode);
			}
...
void CSMSMessageHandler::on_router_alive_report(const char* pBuffer, size_t dwSize, int32_t nSrcAddr)
{
	...
	for (int32_t i = 0; i < msg->nReportCount; i++)
	{
		...
		if (pServerInfo == NULL)
		{
			...
			pRsp->ServerInfo[pRsp->nServerInfoCount++] = _server_info_to_server_data(CurServerInfo);
		}
		else
		{
