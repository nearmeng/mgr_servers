...
BOOL CServiceMgr::init(BOOL bResume)
{
	...
	nRetCode = m_ShmServiceData.init(sstdSMSServiceData, bResume);
	LOG_PROCESS_ERROR(nRetCode);

	nRetCode = m_ShmRouterMgr.init(sstdSMSRouterMgr, bResume);
	LOG_PROCESS_ERROR(nRetCode);
...
void CServiceMgr::mainloop(void)
{
	...
	for (int32_t i = 0; i < m_pShmRouterMgr->nRouterCount; i++)
	{
		...
		if (nCurrTime > rRouterInfo.dwLastPingTime + g_ServerConfig.nHeartBeatTimeOut / 1000 && 
			rRouterInfo.nState != svstDown)
		{
			rRouterInfo.dwCheckDownFlag |= (1 << tbus_get_inst(CMGApp::instance().get_tbus_addr()));

			if (nCurrTime > rRouterInfo.dwLastDownReportTime + g_ServerConfig.nRouterDownReportTimeInterval / 1000)
			{
